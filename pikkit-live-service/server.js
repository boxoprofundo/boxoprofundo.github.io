...
app.get(['/', '/live-summary'], async (req, res) => {
  try {
    ...
    // One URL, two audiences: a browser announces text/html in its Accept
    // header and gets the plain-English view; curl and the voice skill send
    // no such preference and get JSON. So the same bookmark works for Mike
    // and the skill never needed updating. ?format=text/json overrides.
    const wantsHtml = String(req.headers.accept || '').includes('text/html');
    const format = req.query.format || (wantsHtml ? 'text' : 'json');
    if (format === 'text') {
